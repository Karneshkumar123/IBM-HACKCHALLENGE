...
public class AsyncEmailNotificationSenderService implements NotificationSenderService {
	...
	protected class EmailItem implements Runnable {
		...
		public void run() {
			try {
				if(production) {
					...
					LOGGER.debug("Email to {} successful sent"
							, notificationMessage.getDestinationAddress());
				} else {
					LOGGER.warn("DEMO MODE: Email to {}, {}/{}"
							, notificationMessage.getDestinationAddress()
		...
		protected MimeMailMessage buildMessage() throws MessagingException, UnsupportedEncodingException{
			...
			message.setTo(new InternetAddress(notificationMessage.getDestinationAddress()
											, notificationMessage.getDestinationName()));
