	...
	<tbody>
		...
		<c:forEach var="item" items="${items }">
			<tr id="product${item.product.id }" class="item">
				<td class="text-center"><img class="small" src="${item.product.largePhoto }" alt="${item.product.name }"><br>${item.product.name }</td>
				<td class="price">${item.product.price } грн</td>
		...
		<tr>
			...
			<td colspan="${showActionColumn ? 2 : 1}" class="total">${totalCost} грн</td>
		</tr>
	</tbody>
</table>
